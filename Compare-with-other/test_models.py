...
class TestTrajectoryForecasting(unittest.TestCase):
    ...
    def test_recurrent_baseline(self):
        ...
        self.assertEqual(out.shape, (self.B, self.horizon, 3), "LSTM Trajectory shape mismatch.")
    ...
    def test_transformer_baseline(self):
        ...
        out = model(self.dummy_bev)
        self.assertEqual(out.shape, (self.B, self.horizon, 3), "Transformer Trajectory shape mismatch.")
    ...
    def test_mamba_baseline(self):
        try:
            ...
            out = model(self.dummy_bev)
            self.assertEqual(out.shape, (self.B, self.horizon, 3), "Mamba Trajectory shape mismatch.")
        except ImportError:
...
class TestRiskPrediction(unittest.TestCase):
    def setUp(self):
        ...
        # Risk models in this repo expect BEV features (B, T, C, H, W)
        self.dummy_bev = torch.randn(self.B, self.T, self.C, self.H, self.W).to(self.device)
        self.dummy_traj = (torch.rand(self.B, self.horizon, 2) * 2 - 1).to(self.device) # Normalized to [-1, 1]

    def test_mlp_risk(self):
        model = MLPRiskPredictor(in_channels=self.C, hidden_size=self.hidden_size).to(self.device)
        out = model(self.dummy_bev)
        self.assertEqual(out.shape, (self.B, 3), "MLP Risk shape mismatch.")

    def test_occupancy_heatmap(self):
        model = OccupancyHeatmapPredictor(in_channels=self.C).to(self.device)
        risk, heatmap = model(self.dummy_bev, self.dummy_traj)
        # self.assertEqual(heatmap.shape, (self.B, 1, self.H, self.W), "Heatmap shape mismatch.")
        self.assertEqual(risk.shape, (self.B, 3), "Risk shape mismatch.")

    def test_safety_value(self):
        model = SafetyValueEstimator(in_channels=self.C, hidden_size=self.hidden_size).to(self.device)
        out = model(self.dummy_bev)
        self.assertEqual(out.shape, (self.B, 3), "Safety Value shape mismatch.")
