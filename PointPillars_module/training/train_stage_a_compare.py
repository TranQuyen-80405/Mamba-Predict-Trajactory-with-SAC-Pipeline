...
def run_experiment(
    *,
    ...
    wdir = weights_dir or str(log_dir_base / "weights")
...
def main() -> None:
    ...
    ap.add_argument("--lr_neck", type=float, default=3e-5)
    ap.add_argument("--lr_rest", type=float, default=1e-4)
    ...
    ap.add_argument(
        ...
        default="",
        help=(
            "Optional precomputed BEV cache root. If set, all backbones consume "
            "identical cached PointPillars features and skip neck extraction."
        ),
    )
    args = ap.parse_args()
