...
from PointPillars_module.types import DepthPreprocessConfig, PointPillarsConfig  # noqa: E402
...
def _make_stage_a_scheduler(
    ...
    *,
    ...
    """
    Linear warmup for ``warmup_steps`` optimizer steps, then ``CosineAnnealingLR``
    decay to ``eta_min`` over the remaining steps. ``total_optimizer_steps`` is
    set from ``epochs × optimizer_steps_per_epoch``, so cosine spans the full
    training run (e.g. all 30 epochs when ``epochs=30``).
    """
...
def collect_val_metrics(
    ...
    *,
    ...
    infer_n = 0
    infer_ms_total = 0.0
    is_dist = _is_dist_avail_and_initialized()
    with torch.no_grad():
        for batch in loader:
            pts = [
    ...
    if is_dist:
        ...
        local_payload = {
            ...
            "valid": torch.cat(valid_all, dim=0) if valid_all else None,
    ...
    for i, name in enumerate(names):
        ...
        try:
            ...
            out[f"auc_{name}"] = 0.0
    out["brier_risk_avg"] = (
        float(np.mean(brier_scores)) if brier_scores else 0.0
    )
...
def run_stage_a_training(
    ...
    **LR schedule:** linear warmup for ``lr_warmup_iters`` optimizer steps (default 500), then
    cosine decay to ``cosine_eta_min`` over the remaining optimizer steps through the last epoch.

    **Regularization:** ``temporal_dropout`` on Mamba/GRU/LSTM/Transformer stacks; ``risk_label_smoothing``
    on focal risk loss; stronger ``weight_decay`` on AdamW (defaults tuned vs overfitting).
    ...
    **Early stopping:** if ``early_stop_patience > 0``, stop when ``val ap_risk_1s`` does not
    ...
    if bev_cache_root is None or not str(bev_cache_root).strip():
        ...
        if auto_cache.is_dir():
            bev_cache_root = str(auto_cache.resolve())
        else:
            raise FileNotFoundError(
                "BEV cache is now the default training workflow. Missing cache directory: "
                f"{auto_cache}. Build it first with scripts/cache_pointpillars_bev.py "
                f"for data_root={data_path}."
            )

    req_device = device or ("cuda" if torch.cuda.is_available() else "cpu")
    dev = torch.device(req_device)
    world_size_env = int(os.environ.get("WORLD_SIZE", "1"))
    if world_size_env > 1 and not _is_dist_avail_and_initialized():
        backend = "nccl" if dev.type == "cuda" else "gloo"
        dist.init_process_group(backend=backend)
    is_dist = _is_dist_avail_and_initialized()
    rank = dist.get_rank() if is_dist else 0
    world_size = dist.get_world_size() if is_dist else 1
    if is_dist and dev.type == "cuda":
    ...
    val_stats = val_ds.horizon_label_stats()
    missing_pos = [
        k for k, s in val_stats.items() if int(s["valid"]) > 0 and int(s["positive"]) == 0
    ]
    if missing_pos:
        msg = (
            f"validation split has zero positive samples for horizons: {missing_pos}. "
            f"stats={val_stats}. Regenerate/re-split data to avoid degenerate AP/AUC."
        )
        if is_dist:
            dist.barrier()
    ...
    if is_dist:
        dist.barrier()
    writer: Union[SummaryWriter, _NullSummaryWriter]
    writer = SummaryWriter(str(run_dir)) if is_main else _NullSummaryWriter()
    ...
    if is_main and metrics_jsonl.is_file():
    ...
    temporal_trainable = _trainable_param_count(core_model.mamba)
    ...
    if is_main:
        ...
        log_mamba_temporal_runtime(core_model.mamba, device=dev)

    tail_params: List[torch.nn.Parameter] = (
        list(core_model.reducer.parameters())
        + list(core_model.mamba.parameters())
        + list(core_model.head.parameters())
        + list(core_model.traj_head.parameters())
    )
    neck_params: List[torch.nn.Parameter] = list(core_model.pp.model.neck.parameters())
    ...
    train_start_s = time.perf_counter()
    params_m = float(temporal_trainable) / 1_000_000.0
    if dev.type == "cuda":
    ...
    for epoch in range(epochs):
        ...
        core_model.reducer.train()
        core_model.mamba.train()
        core_model.head.train()
        core_model.traj_head.train()
        if phase == "a2":
            core_model.pp.model.neck.train()
        epoch_loss = 0.0
        n_batches = 0
        ...
        for micro_i, batch in enumerate(train_loader):
            ...
            loss_r = focal_bce(
                ...
                label_smoothing=float(risk_label_smoothing),
            )
            ...
            global_micro_step += 1
            if is_main and ((micro_i + 1) % 100 == 0):
                print(
                    f"[{rn}] epoch {epoch + 1}/{epochs} "
                    f"micro_batch {micro_i + 1}/{n_micro} "
                    f"loss={float(total_mb.detach()):.4f}",
                    flush=True,
                )
        ...
        if is_dist:
            loss_buf = torch.tensor(
                [epoch_loss, float(n_batches)], dtype=torch.float64, device=dev
            )
            dist.all_reduce(loss_buf, op=dist.ReduceOp.SUM)
            epoch_loss = float(loss_buf[0].item())
            n_batches = int(loss_buf[1].item())
        ...
        metrics = collect_val_metrics(core_model, val_loader, dev)
        ...
        if not np.isnan(ap1):
            if ap1 > best_ap_for_stop + es_delta:
                ...
                best_state_dict = _cpu_state_dict(core_model)
                epochs_no_improve = 0
            elif es_patience > 0:
                epochs_no_improve += 1
        elif es_patience > 0:
        ...
        for k, v in metrics.items():
            out_key = k if str(k).startswith("val_") else f"val_{k}"
            row_epoch[out_key] = float(v) if isinstance(v, (float, int)) else v
        peak_gpu_gb = (
            float(torch.cuda.max_memory_allocated(dev)) / float(1024 ** 3)
            if dev.type == "cuda"
            else float("nan")
        )
        train_time_hours = (time.perf_counter() - train_start_s) / 3600.0
        row_epoch["params_M"] = params_m
        row_epoch["peak_gpu_gb"] = peak_gpu_gb
        row_epoch["train_time_hours"] = train_time_hours
        if dev.type == "cuda":
            torch.cuda.reset_peak_memory_stats(dev)
        writer.add_scalar("train/params_M", params_m, epoch)
        if not np.isnan(peak_gpu_gb):
            writer.add_scalar("train/peak_gpu_gb", peak_gpu_gb, epoch)
        writer.add_scalar("train/elapsed_hours", train_time_hours, epoch)
    ...
    writer.close()
    # All ranks must enter the same barriers. A barrier only on non-main ranks deadlocks
    # or desynchronizes NCCL when rank-0 does checkpoint I/O before the next model run.
    if is_dist:
        dist.barrier()
    ...
    if is_main:
        ...
        wdir_base = Path(weights_dir) if weights_dir else log_dir_base / "checkpoints"
        if save_weights:
            stem = f"{backbone}_risk_{int(epochs_run)}epochs"
            wdir = wdir_base / stem
            wdir.mkdir(parents=True, exist_ok=True)
            run_stem = f"{stem}_{rn}"
            latest_stem = (
                f"{backbone}_risk_a2_latest"
                if phase == "a2"
                else f"{backbone}_risk_latest"
            )
            out_ts = wdir / f"{run_stem}_last.pt"
            out_latest = wdir / f"{latest_stem}.pt"
            last_sd = _cpu_state_dict(core_model)
            payload_last = {
            ...
            src_ckpt = Path(ckpt_path)
            if src_ckpt.is_file():
                copied_src = wdir / f"{stem}_source{src_ckpt.suffix}"
                shutil.copy2(src_ckpt, copied_src)
                row["checkpoint_source_copy"] = str(copied_src.resolve())

            if best_state_dict is not None:
                out_best = wdir / f"{run_stem}_best_val_ap.pt"
                payload_best = {
                    "format": "stage_a_full_pipeline_risk_traj_v1",
                    "stage": phase,
                    "backbone": backbone,
                ...
                best_latest = (
                    f"{backbone}_risk_a2_best_val_ap.pt"
                    if phase == "a2"
                    else f"{backbone}_risk_best_val_ap.pt"
                )
    ...
    if not is_main:
        row_non_main: Dict[str, Union[float, str]] = {
            k: float(v) for k, v in report_metrics.items()
        }
        row_non_main["backbone"] = backbone
        row_non_main["stage"] = phase
        row_non_main["run_name"] = rn
        row_non_main["epochs_run"] = float(epochs_run)
        row_non_main["early_stopped"] = early_stopped
        row_non_main["best_ap_risk_1s"] = best_ap_for_stop
        row_non_main["log_dir"] = str(run_dir.resolve())
        row_non_main["tensorboard_logdir"] = str(run_dir.resolve())
        return row_non_main
...
def build_arg_parser(*, fixed_backbone: Optional[str] = None) -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        type=float,
        default=5e-5,
        help="AdamW LR for tail params (a1) or lr_rest group (a2); default reduced for stability.",
    )
    ap.add_argument(
        "--weight_decay",
        type=float,
        default=0.05,
        help="AdamW weight decay (default 0.05 for stronger L2 regularization).",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=5e-6,
        help="AdamW LR for pp.neck when --stage a2 (scaled with --lr).",
    )
    ap.add_argument(
        "--lr_rest",
        type=float,
        default=1.67e-5,
        help="AdamW LR for reducer+temporal+heads when --stage a2 (scaled with --lr).",
    )
    ...
    ap.add_argument(
        ...
        default="",
        help=(
            "Root directory of precomputed BEV cache created by "
            "scripts/cache_pointpillars_bev.py. If omitted, defaults to "
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.05,
        help="Label smoothing for focal BCE on risk heads (0 disables).",
    )
    ap.add_argument(
        "--temporal_dropout",
        type=float,
        default=0.1,
        help="Dropout on temporal stacks (Mamba/GRU/LSTM; Transformer uses max(this, 0.1)).",
    )
    return ap
